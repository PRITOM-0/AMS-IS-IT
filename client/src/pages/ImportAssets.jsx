...
const ImportAssets = () => {
    ...
    reader.onload = (loadEvent) => {
      try {
        ...
        if (!validColumns.length || !validRows.length) {
          setSuccess(false);
          setMessage("No valid data found. Empty column names and blank rows were removed.");
          setLoading(false);
          return;
        }
  ...
  const validateAndProceed = () => {
    ...
    const finalAssets = excelData.map((row, index) => buildAssetFromRow(row, index, mapping));
    
    // Navigate to StoreAssets page and pass cleaned asset objects in route state
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-8 sm:p-14 shadow-sm relative overflow-hidden">
        ...
        <div className="max-w-md mx-auto text-center">
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-2 leading-relaxed">
            Supports <span className="font-semibold text-indigo-600">.xlsx</span>, <span className="font-semibold text-indigo-600">.xls</span>, or <span className="font-semibold text-indigo-600">.csv</span>. Blank rows and header spaces are auto-cleaned.
          </p>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="p-4 sm:px-6 border-t border-slate-100 bg-slate-50/60 flex items-center justify-between">
          <p className="text-xs text-slate-500 hidden sm:block">
            Map <span className="font-semibold text-indigo-600">Asset Code</span> to proceed.
          </p>
          <button
