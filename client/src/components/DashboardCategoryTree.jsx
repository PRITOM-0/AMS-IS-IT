...
export function DashboardCategoryTree({ equipmentTree }) {
  ...
  // Location toggle
  // -----------------------------------------
  const toggleLocation = (company, location) => {
    const key = `${company}__${location}`;
  ...
  const categoryBreakdown = useMemo(() => {
    ...
    Object.values(equipmentTree || {}).forEach((locations) => {
      Object.values(locations || {}).forEach((departments) => {
        Object.values(departments || {}).forEach((equipmentObj) => {
          Object.entries(equipmentObj || {}).forEach(
            ([equipment, count]) => {
              counts[equipment] =
                (counts[equipment] || 0) + Number(count || 0);
            }
          );
        });
      });
  ...
  const totalGlobalAssets = useMemo(() => {
    return categoryBreakdown.reduce(
      (acc, [_, count]) => acc + Number(count || 0),
      0
    );
  }, [categoryBreakdown]);
      ...
      <div className="rounded-3xl border rounded-xl shadow-sm p-4 hover:shadow-md transition duration-200 border-green-200 text-indigo-700 bg-gradient-to-br from-green-200 via-white to-violet-200 shadow-[0_20px_45px_-25px_rgba(15,23,42,0.30)]">
        ...
        <div className="mb-3 border-b border-slate-800 pb-2.5">
          ...
          <p className="text-[11px] font-medium text-slate-500">
            Click any company, location, department, or equipment type to
            view filtered assets.
          </p>
        </div>
        ...
        <div className="space-y-2.5">
          {Object.entries(equipmentTree || {}).map(
            ([company, locations]) => {
              const isCompanyExpanded =
                !!expandedCompanies[company];

              // Total assets inside this company
              const companyTotal = Object.values(
                locations || {}
              ).reduce(
                (locationAcc, departments) =>
                  locationAcc +
                  Object.values(departments || {}).reduce(
                    (departmentAcc, equipmentObj) =>
                      departmentAcc +
                      Object.values(equipmentObj || {}).reduce(
                        (equipmentAcc, count) =>
                          equipmentAcc + Number(count || 0),
                        0
                      ),
                    0
                  ),
                    ...
                    <div className="space-y-2 border-t border-slate-300/80 bg-slate-200/40 p-2.5">
                      {Object.entries(locations || {}).map(
                        ([location, departments]) => {
                          const locationKey = `${company}__${location}`;

                          const isLocationExpanded =
                            !!expandedLocations[locationKey];

                          // Location total
                          const locationTotal = Object.values(
                            departments || {}
                          ).reduce(
                            (acc, equipmentObj) =>
                              acc +
        ...
        <div className="mb-3 flex items-center justify-between border-b border-slate-800 pb-2.5">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] font-medium text-slate-500">
                Aggregated equipment counts across all companies,
          ...
          {categoryBreakdown.map(
            ([category, totalCount]) => {
              const percentage = totalGlobalAssets
                ? Math.round(
                    (totalCount / totalGlobalAssets) * 100
                  )
                : 0;
